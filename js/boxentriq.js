...
var Boxentriq = {
    ...
    Tapcode: {
        enc: function (txt, mode) {
            ...
            for (var i = 0; i < src.length; i++) {
                ...
                if (letter == ' ') {
                    ...
                    if (code != null) {
                        if (codeType == 'dots')
                            dst += this.numToDots(code.row) + ' ' + this.numToDots(code.col) + '  ';
                        else
        ...
        dec: function (txt, mode) {
            ...
            if (matches != null) {
                for (var i = 0; i < matches.length - 1; i += 2) {
                    ...
                    var l2 = matches[i + 1];
                    if (codeType == 'dots')
                        dst += this.posToLetter(this.dotsToNum(l1), this.dotsToNum(l2));
                    else
                        dst += this.posToLetter(parseInt(l1), parseInt(l2));
                }
            }
            return dst;
        },

        letterToPos: function (letter) {
            if (letter == 'k')
                letter = 'c';
        ...
        numToDots: function (num) {
            return '.'.repeat(num);
        },

        dotsToNum: function (dots) {
            var i;
